...
\question{(Individual) Why might we be interested in a ``subset'' F test?}

\begin{minipage}[l][2cm][c]{\textwidth}
\begin{comment}
%\note{We may wish to know if a group of predictors have are significantly related to the response variable, \textit{after accounting for} another set of variables that are already in the model. }
\end{comment}
\end{minipage}
...
\question{(Groups) Consider $H_0$ : $\beta_2=\beta_3=0$. How would you describe this hypothesis in an English sentence?}
...
\question{(Groups) What other advantages (besides help with multicollinearity) might standarizing our variables provide us?}
