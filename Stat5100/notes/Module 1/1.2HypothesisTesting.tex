...
\question{Why is the assumed distribution for the test-statistic such a big deal?}

\begin{minipage}[l][3cm][c]{\textwidth}
%\begin{comment}
\note{We use the assumed probability distribution to calculate the p-value, and the p-value is how we determine the ``significance'' of our results. If the probability distribution is not appropriate then the p-value will be worthless. }
%\end{comment}
\end{minipage}
...
We will never know the values of $\mu_1$ and $\mu_2$. They are \textbf{population parameters} that we could only know if we sampled every unit in the population (which, in this case, would be all bamboo shoots grown in the two lighting conditions).
...
\question{Suppose the p-value for the above example had been 0.02 instead of 0.006. Would your conclusions change? What about if the p-value had been 0.13? How about 0.98?}
