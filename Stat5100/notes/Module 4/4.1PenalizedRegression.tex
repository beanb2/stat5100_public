...
\newpage
\question{(Why might we be OK with giving up unbiasedness in order to minimize variance?}
...
\item Biased, yet stable, estimates of the coefficients can often provide greater predictive accuracy than an OLS model. 
\ei
}
%\end{comment}
\end{minipage}
...
\begin{itemize}
  ...
  \end{eqnarray}
  \begin{itemize}
  \item Where $\tilde{b}_k$ represents some initial estimate of the model coefficients (perhaps using OLS or traditional LASSO). 
  \end{itemize}
   \item Elastic Net
  \begin{eqnarray}
