...
tinder <- read.csv("../../data_csv/tinder.csv")
...
# We see a few warnings above because there were some missing values coded as
...
tinder_baseline_pred <- mean(tinder_train$Genuine)
@
...
tinder_test_pred3 <- rep(tinder_baseline_pred, times = nrow(tinder_test))
