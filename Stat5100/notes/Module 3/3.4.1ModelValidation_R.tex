...
\hlcom{# We see a few warnings above because there were some missing values coded as}
\hlcom{# "." inside of the csv file.}
...
\hlstd{tinder_baseline_pred} \hlkwb{<-} \hlkwd{mean}\hlstd{(tinder_train}\hlopt{$}\hlstd{Genuine)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hlstd{tinder_test_pred3} \hlkwb{<-} \hlkwd{rep}\hlstd{(tinder_baseline_pred,} \hlkwc{times} \hlstd{=} \hlkwd{nrow}\hlstd{(tinder_test))}
